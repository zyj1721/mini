...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
        auto  chang=analyseConstantDeclaration();
        if(chang.has_value())
            return chang;
		// <变量声明>
		auto bian=analyseVariableDeclaration();
		if(bian.has_value())
            return  bian;
              ...
              //  addUninitializedVariable(next.value());
                ...
                */
                ...
                if (next.has_value() && next.value().GetType()==TokenType ::SEMICOLON)//分号
                {
                 ...
                 //   if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
                 //       return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
                    addUninitializedVariable(nextvar.value());
                    _instructions.emplace_back(Operation::LIT, 0);
                }
                else if (next.has_value()&&next.value().GetType()==TokenType::EQUAL_SIGN){//是等号
                    // '<表达式>'
                    _instructions.emplace_back(Operation::LIT, 0);
                    next=nextToken();
                    auto biao=analyseExpression();
                    if(biao.has_value())
                        return biao;
                    addVariable(nextvar.value());
                    if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
                        return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
                ...
                if(next.value().GetType()!=TokenType::EQUAL_SIGN)//如果没有等于
                {
                    ...
                    {
                        addUninitializedVariable(nextvar);
                        /*_instructions.emplace_back(Operation::LIT, 0);//未定义 占空间
                        return{};*/
                        continue;
                    }
                    return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
...
// 需要补全
    ...
    if(!next.has_value())
        return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIncompleteExpression);
    if (next.value().GetType() == TokenType::UNSIGNED_INTEGER){
        //next.value().GetValueString() >> temp;
        //temp=atoi(next.value().GetValueString());
        temp=std::any_cast<int >(next.value().GetValue());
        out=x*temp;
    } else{
        //if(x==-1 &&next.value())
...
std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
    // 这里除了语法分析以外还要留意
    auto biaoshi=nextToken();
    auto temp=biaoshi;
   // if(!biaoshi.has_value()||biaoshi.value().GetType()!=TokenType::IDENTIFIER)
     //   return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedIdentifier);
    // 标识符声明过吗？
    if(!isDeclared(biaoshi.value().GetValueString()))
        return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNotDeclared);
    // 标识符是常量吗？
    if(isConstant(biaoshi.value().GetValueString()))
    ...
    if(biao.has_value())
        return biao;
    //
    if(!isInitializedVariable(biaoshi.value().GetValueString()))
    {
        int t=_uninitialized_vars[biaoshi.value().GetValueString()];
        addVariable(biaoshi.value());
        _vars[biaoshi.value().GetValueString()]=t;
        _uninitialized_vars.erase(biaoshi.value().GetValueString());
    }

    _instructions.emplace_back(Operation::STO,getIndex(biaoshi.value().GetValueString()));
        next = nextToken();
    ...
    switch (next.value().GetType()) {
        ...
        case TokenType ::UNSIGNED_INTEGER:
        {
         ...
         //   next.value().GetValueString()>>tem;
            tem=std::any_cast<int >(next.value().GetValue());
            //printf("%d",tem);
